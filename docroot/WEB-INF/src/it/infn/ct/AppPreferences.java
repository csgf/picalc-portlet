        ...
        for(int i=0,h=0; i<getNumInfrastructures(); i++) {
            ...
            if(appInfrastructuresInfo.get(i).getEnableInfrastructure().equalsIgnoreCase("yes")) {
                ...
                if(getAcronymInfrastructure(i).equalsIgnoreCase("unicore")) {
                    ...
                    // * Password      -> bdiiHost second field of the given ';' separated string
                    // * JSAGA adaptor -> wmsHosts
                    String sshItems[]  = getBdiiHost(i).split(";");
                    ...
                    if(numSSHItems>2)    queue=sshItems[2].trim();

                    // Build the GridEngine' infrastructure object and assign it to the infrastructures array
                    infrastructuresInfo[h++] = new InfrastructureInfo( getAcronymInfrastructure(i)+";"+queue
                                                                      ,"ssh"
                                                                      ,username
                                                                      ,password
                                                                      ,wmsHostList);
                    if(_log !=null)
                        _log.info("SSH resources: "
                               +LS+"      username: '" + username    +"'"
                               +LS+"      password: '" + password    +"'"
                               +LS+"         queue: '" + queue       +"'"
                               +LS+" JSAGA adaptor: '" + wmsHostList +"'"
