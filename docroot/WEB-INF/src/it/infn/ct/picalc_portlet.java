 ...
 * @author <a href="mailto:riccardo.bruno@ct.infn.it">Riccardo Bruno</a>(COMETA)
 */
public class picalc_portlet extends GenericPortlet {        
    
    // Instantiate the logger object
    AppLogger _log = new AppLogger(picalc_portlet.class);
    ...
    class AppInput {
        // Applicatoin inputs
        String cpuNumber;       // Number of CPUs for MPI
        ...
        /**
         * AppInput class stores temporary files
         * this method removes these files upon 
         * the GC frees this object
         *
        protected void finalize() {
            if (inputSandbox_inputFile != null) {
               try{
                   // !!! Job execution may require a bit of time to complete
                   // before to remove temporary files used by the jobSubmit()
                   // it is better to wait a bit (30 min default)
                   Thread.currentThread().sleep(30*60*1000);// (30-min)*(60-sec)*(1-sec)
                   // Notify the file cleanup
                   _log.info(
                          LS+"Freeing AppInput:"
                         +LS+"----------------"
                         +LS+"Removing: "+inputSandbox_inputFile
                           ); 
                   File f = new File(inputSandbox_inputFile);
                   if(!f.delete())
                       _log.error("Unable to delete PortalInfo file: "+inputSandbox_inputFile);
               }
        ...
        throws PortletException, IOException
    {
        ...
        try {
            ...
            emailAddress = user.getDisplayEmailAddress();
            ...
            _log.info(
                  ...
                  +LS+"portletName   : '"+portletName  +"'"  
                );                                                        
        } catch(PortalException pe) {
            _log.error("Unable to retrieve portal data: '"+pe.toString()+"'");
            return;
        } catch(SystemException se) {
            _log.error("Unable to retrieve portal data: '"+se.toString()+"'");
            return;
        }
        ...
        if (mode.equals(PortletMode.VIEW)) {
            ...
            switch(Actions.valueOf(actionStatus)) {
                ...
                case ACTION_PILOT:
                    ...
                    // Stores the new pilot script remving eventually \r characters
                    String pattern = "[\r]";
                    
                    String pilotScript=request.getParameter("pilotScript").replaceAll(pattern, "");
                    String mpiStart   =request.getParameter("mpiStart"   ).replaceAll(pattern, "");
                    String mpiHooks   =request.getParameter("mpiHooks"   ).replaceAll(pattern, "");
                    
                    storeString(appServerPath+"WEB-INF/job/"+appPreferences.getPilotScript(),pilotScript);
                    storeString(appServerPath+"WEB-INF/job/mpi-start-wrapper.sh"            ,mpiStart);
                    storeString(appServerPath+"WEB-INF/job/mpi-hooks.sh"                    ,mpiHooks);
                ...
                case ACTION_SUBMIT:
                    ...
                    // Instanciate the PortalInfo class
                    PortalInfo portalInfo=
                           new PortalInfo( _log
                                          ...
                                          ,username
                                          ,emailAddress
                                          ,Integer.parseInt(appPreferences.getGridOperationId())
                                          ,portletName
                                          ,appInput.jobIdentifier
                                          ,appInput.notifyEmail
                                          ,""+appInput.notifyStart
                                          ,""+appInput.notifyStop
                                          ,""
                                          );
                                        
                    // Submit the job
                    submitJob(appInput,portalInfo);
        ...
        //----------
            ...
            if(pref_action.equalsIgnoreCase("next")) {  
                ...
                response.setRenderParameter("mpiStart"
                                            ,updateString(appServerPath+"WEB-INF/job/mpi-start-wrapper.sh"            ));                
                response.setRenderParameter("mpiHooks"
                                            ,updateString(appServerPath+"WEB-INF/job/mpi-hooks.sh"                    ));                
            }
            else {
    ...
    throws PortletException, IOException 
    {
        ...
        switch(Views.valueOf(currentView)) {
            ...
            case VIEW_PILOT: {
                ...
                String mpiStart = request.getParameter("mpiStart");
                request.setAttribute("mpiStart", mpiStart);                
                String mpiHooks = request.getParameter("mpiHooks");                               
                request.setAttribute("mpiHooks", mpiHooks);
    ...
    throws PortletException,IOException {
        ...
        // For MPI wrapper script names are fixed to: 
        request.setAttribute("pref_mpiStart"       ,"mpi-start-wrapper.sh");
        request.setAttribute("pref_mpiHooks"       ,"mpi-hooks.sh"        );
    ...
    */
    ...
    throws PortletException, IOException{
        ...
        if(prefs!=null) {            
            ...
            String jobRequirements=appPreferences.getJobRequirements();
            String pilotScript    =appPreferences.getPilotScript    ();            
    ...
    private enum inputControlsIds {
         CpuNumber         // Number of CPUs for MPI
        ,JobIdentifier     // User defined Job identifier
       //,notifyEmail       // User notification email
       //,notifyStart       // User notification Start flag
       //,notifyStop        // User notification Stop  flag
    };
    /**
    ...
    if (PortletFileUpload.isMultipartContent(request))
        ...
        _log.info(
              ...
              +LS+"-----------------------"
              +LS+"CpuNumber    : '"+appInput.cpuNumber    +"'"  
    ...
    void processInputFile(FileItem item,AppInput appInput) {            
        /* There are no input files to deal with ...
    ...
    void updateFiles(AppInput appInput) {
        ...
        try {            
            appInput.inputSandbox_inputFile="/tmp/"
                                                ...
                                                +"_input_file.txt"
                                                ;
            storeString(appInput.inputSandbox_inputFile
                       ,appInput.inputFileText);            
        }
        catch (Exception e) {
    ...
    void submitJob(AppInput appInput, PortalInfo portalInfo) {
        ...
        if(infrastructuresInfo.length > 0) {            
            ...
            portalInfo.setMiddleware(infrastructure.getMiddleware());
            _log.info(LS+"----------------------------------"
                     +LS+"MULTIMIDDLEWARE: "+infrastructure.getMiddleware()
                     +LS+"----------------------------------");
		...
		miJobSubmission.setExecutable(executable);
                miJobSubmission.setTotalCPUCount(cpunumber);
                miJobSubmission.setArguments(arguments);   
		...
		// Infrastructure specific settings have been specified
                ...
                //WSGRAM MPI
                miJobSubmission.setExecutable("cpi_xn03");
                miJobSubmission.setSPMDVariation("MPI");
               ...
               miJobSubmission.setExecutable("hello.mpi");
               miJobSubmission.setSPMDVariation("OpenMPI");
               miJobSubmission.setTotalCPUCount("4");
               ...
               readyToSubmit=true;
                ...
                if(nameItems.length > 1) {
                        ...
                        if(infraQatH.length > 0) infrastructureQueue = infraQatH[0];
                        if(infraQatH.length > 1) infrastructureHost  = infraQatH[1];
                        _log.info(LS+"----------------------"
                                 +LS+"SSH:"
                                 +LS+"    infraQueue: " + infrastructureQueue
                                 +LS+"    infraHost : " + infrastructureHost
                                 +LS+"----------------------"
                                 );
                 }
                 // Prepare job description
		 miJobSubmission.setExecutable("start.sh");
                 ...
                 inputSandbox=appServerPath+"WEB-INF/job/start.sh"
                         +","+appServerPath+"WEB-INF/job/lsf.sh"
                       //+","+appServerPath+"WEB-INF/job/mpi.job" automatically generated by start.sh
                         +","+appServerPath+"WEB-INF/job/hello.mpi"
                       //+","+appServerPath+"WEB-INF/job/cpi.mpi"
                         ;
	         // Infrastructure specific settings have been specified
                 // the job is ready to be submitted
                 readyToSubmit=true;	
            ...
            // } // if supported middleware (ourgrid)
                ...
                _log.warn(
                       LS+"The selected middleware: '"+infrastructure.getMiddleware()+"' is unsupported!"
                         );
            }
            ...
            if(readyToSubmit) {
                ...
                // Submit the job
                //
                miJobSubmission.submitJobAsync(infrastructure,appInput.username, portalIPAddress, applicationId, appInput.jobIdentifier);
